...
`production` environment variable `AURA_MINISIGN_PUBLIC_KEY`. Store the
secret in the same environment as an environment-scoped secret. The workflow
waits for any configured environment protection rules before writing the secret
