...
| [`docs/releases/`](docs/releases/)   | Release notes (`0.1.0-alpha`; current `0.1.1-alpha.8`)                       |
...
Homepage, user docs, and RFC catalog: **https://aura.pilotworks.dev/** (Cloudflare Pages).
...
### Compiler (through C21 + S2 toolchain — borrow/ref, captures, collections, and DX)
...
cargo run -p aura-cli -- run corpus/std_collections/scoped_borrow_iteration # scoped ref iteration checks (C21e)
...
- **Collections C20f–j** Snapshot iterator contract, `Array<Interface>`, key-based `HashMap.entry(key)` handles, invalidation-checked live entry views, and live HashMap/HashSet cursors are shipped
...
- **S2:** verified HTTPS/nested locked registry consumption, direct Git-origin package resolution, Unix artifacts, installer safety, acceptance wiring, and the `0.1.1-alpha.8` release are complete; tier-2/native evidence remains a release-host gate ([plan](docs/plans/2026-07-21-s2-production-toolchain.md)). The registry is origin-first and Go-module-inspired: publication is immutable Git tag/push, lockfiles pin source/revision/checksum, and the versioned proxy read contract is implemented as an Aura C-backend example while CLI integration remains separate.
- **C22 / v0.1.1-alpha.8:** async/task syntax, borrow barriers, deterministic task frames/executor, bounded channels, typed channel lowering, corpus, diagnostics, ownership audits, and the bounded release scope are shipped. General async lowering, richer captures/outcomes, typed async HTTP handles, public-host registry rehearsal, and tier-2/native evidence remain partial; CLI proxy integration remains deferred while the Aura C-backend example is available. See the [C22 plan](docs/plans/2026-07-22-next-20-c22-async-tasks.md) and [v0.1.1-alpha contract](docs/plans/v0.1.1-alpha/contract-matrix.tsv).
...
Array capture still uses a captured payload/header representation. The shipped
scoped `ref T` MVP now rejects escaping borrows and makes Array field returns
non-moving, but live views, mutable borrows, and owner-movement guarantees for
escaping closures remain deferred.
