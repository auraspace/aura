...
fn invalid_linker_option_surfaces_before_false_executable() {
    ...
    options.profile_settings.linker = Some(format!("aura-missing-linker-{}", std::process::id()));
...
"#,
    ...
    let generated = emit_c_from_ast(&file).expect("emit no-await class parameter frame fixture");
...
"#,
    ...
    let generated = emit_c_from_ast(&file).expect("emit struct heap-class task outcome fixture");
...
"#,
    ...
    let generated = emit_c_from_ast(&file).expect("emit no-await open generic identity fixture");
...
"#,
    ...
    let generated = emit_c_from_ast(&file).expect("emit async class method branch-loop fixture");
    assert!(generated.contains("aura async general CFG Unit lowering"));
...
"#,
    ...
    let generated = emit_c_from_ast(&file).expect("emit async class await-while-condition fixture");
    assert!(generated.contains("aura async general CFG Unit lowering"));
    assert!(generated.contains("__aura_async_cond_"));
...
"#,
    ...
    let bin = dir.join(&stem);
    let generated_c = dir.join(format!("{stem}.aura.c"));
    build_from_file(&file, &bin, &root.join("runtime/runtime.c")).expect("compile std.tls fixture");
    let _ = fs::remove_file(bin);
    let _ = fs::remove_file(generated_c);
}
...
"#,
    ...
    let bin = dir.join(&stem);
    let generated_c = dir.join(format!("{stem}.aura.c"));
    build_from_file(&file, &bin, &root.join("runtime/runtime.c")).expect("compile std.log fixture");
...
"#,
    ...
    let bin = dir.join(&stem);
    let generated_c = dir.join(format!("{stem}.aura.c"));
    build_from_file(&file, &bin, &root.join("runtime/runtime.c")).expect("compile vararg fixture");
...
"#,
    ...
    assert!(generated.contains("aura_var_std_io_Result_std_io_Payload_std_io_TaskError_OkOwned"));
    let root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .and_then(|path| path.parent())
        .expect("workspace root");
...
"#,
    ...
    let generated = emit_c_from_ast(&file).expect("emit nested typed class failure join fixture");
...
"#,
    ...
    let generated = emit_c_from_ast(&file).expect("emit nested-field async class catch fixture");
...
"#,
    ...
    let bin = dir.join(&stem);
    let generated_c = dir.join(format!("{stem}.aura.c"));
    build_from_file(&file, &bin, &root.join("runtime/runtime.c")).expect("compile std.fs fixture");
...
"#,
    ...
    let bin = dir.join(&stem);
    let generated_c = dir.join(format!("{stem}.aura.c"));
    build_from_file(&file, &bin, &root.join("runtime/runtime.c")).expect("compile std.os fixture");
...
fn compiles_immediate_await_through_frame_polling() {
    ...
    build_from_file(&file, &bin, &root.join("runtime/runtime.c")).expect("compile immediate await");
...
"#,
    ...
    assert!(
        generated.contains("aura_task_executor_release(__aura_task_executor, &data->await_task_0)")
            || generated.contains("aura async general CFG Int lowering")
    );
    assert!(
        generated.contains("aura_task_executor_release(__aura_task_executor, &data->await_task_3)")
            || generated.contains("aura async general CFG Int lowering")
    );
...
"#;
    ...
    assert!(generated.contains("aura_task_frame_wait_on(frame, data->await_task)"));
    assert!(!generated.contains("aura_task_executor_wake(__aura_task_executor, data->await_task)"));
    assert!(!generated.contains("aura_task_executor_run_one(__aura_task_executor)"));
    assert!(generated.contains("aura_task_frame_propagate_error(frame, data->await_task)"));
    let root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent()
...
"#
    ...
    let generated = emit_c_from_ast(&file).expect("emit sync foreign handle throw/catch fixture");
...
"#
    ...
    assert!(generated.contains("aura_var_std_io_Result_ForeignHandle_Int_std_io_TaskError_OkOwned"));
...
"#;
    ...
    let generated = emit_c_from_ast(&file).expect("emit repeated caller-owned Array CFG fixture");
    assert!(generated.contains("aura async general CFG Array lowering"));
    assert!(generated.contains("data->await_task_owned = false"));
    assert!(generated.contains("aura_task_frame_propagate_error(frame, data->await_task)"));
...
"#,
    ...
    assert!(generated.contains("aura_task_frame_set_resume_state(frame, 2)"));
    assert!(
        generated.contains("data->await_task_1") || generated.contains("aura async general CFG")
    );
    assert!(generated.contains("aura_gc_collect"));
    assert!(
        generated.contains("aura_task_frame_propagate_error(frame, data->await_task_0)")
...
"#,
    ...
    let generated = emit_c_from_ast(&file).expect("emit general CFG String for-in await fixture");
    assert!(generated.contains("aura async general CFG Int lowering"));
...
"#,
    ...
    assert!(generated.contains("aura_task_frame_set_resume_state(frame, 3)"));
    assert!(
        generated.contains("data->await_task_2") || generated.contains("aura async general CFG")
    );
    assert!(generated.contains("aura_gc_collect"));
...
"#,
    ...
    assert!(generated.contains("aura_task_frame_set_resume_state(frame, 4)"));
    assert!(
        generated.contains("data->await_task_3") || generated.contains("aura async general CFG")
    );
    assert!(
        generated.contains("aura_task_executor_release(__aura_task_executor, &data->await_task_3)")
            || generated.contains("aura async general CFG")
...
"#,
    ...
    assert!(generated.contains("aura_task_frame_propagate_error(frame, data->await_task)"));
    assert!(
        generated.contains("aura_task_executor_release(__aura_task_executor, &data->await_task)")
    );
...
"#,
    ...
    let generated = emit_c_from_ast(&file).expect("emit string array match-await binding fixture");
    assert!(generated.contains("aura async general CFG Array lowering"));
    assert!(generated.contains("__match_value = choice.data.First.value"));
    assert!(generated.contains("data->seed.data"));
...
"#,
    ...
    let generated = emit_c_from_ast(&file).expect("emit general four-await String array fixture");
    assert!(generated.contains("aura async general suspension state=4"));
    assert!(generated.contains("aura_method_Array_String_clone"));
    assert!(generated.contains("aura_gc_collect"));
...
"#,
    ...
    let generated = emit_c_from_ast(&file).expect("emit all bounded capture kinds across await");
...
fn builds_and_runs_bounded_array_local_capture() {
    ...
    assert!(generated.contains("__spawn_data->captured = aura_method_Array_Int_clone(&captured);"));
    let output = Command::new(&bin)
        .output()
...
"#,
    ...
    let generated = emit_c_from_ast(&file).expect("emit inferred Array capture CFG spawn fixture");
    assert!(generated.contains("aura_fn_demo___spawn_cfg_"));
    assert!(generated.contains("aura_method_Array_Int_clone"));
    let root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent()
...
"#,
    ...
    let generated = emit_c_from_ast(&file).expect("emit inferred class capture CFG spawn fixture");
    assert!(generated.contains("aura_fn_demo___spawn_cfg_"));
    assert!(
...
"#,
    ...
    let generated = emit_c_from_ast(&file).expect("emit mutable String general CFG spawn fixture");
    assert!(generated.contains("aura_fn_demo___spawn_cfg_"));
...
"#,
    ...
    let generated = emit_c_from_ast(&file).expect("emit mutable Array general CFG spawn fixture");
    assert!(generated.contains("aura_fn_demo___spawn_cfg_"));
...
"#,
    ...
    let generated = emit_c_from_ast(&file).expect("emit mutable class general CFG spawn fixture");
    assert!(generated.contains("aura_fn_demo___spawn_cfg_"));
    assert!(generated.contains("aura_box_ptr_retain"));
    assert!(generated.contains("aura_box_ptr_release"));
    let root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
...
"#,
    ...
    assert!(generated.contains("aura_var_std_io_Result_Opt_std_io_Box_std_io_TaskError_OkOwned"));
    let root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .and_then(|path| path.parent())
        .expect("workspace root");
...
"#,
    ...
    let bin = dir.join(&stem);
    let generated_c = dir.join(format!("{stem}.aura.c"));
    build_from_file(&file, &bin, &root.join("runtime/runtime.c")).expect("compile select fixture");
...
"#,
    ...
    let generated = emit_c_from_ast(&file).expect("emit immutable Array lambda capture fixture");
    assert!(generated.contains("aura_method_Array_Int_clone(&values)"));
...
"#,
    ...
    let generated = emit_c_from_ast(&file).expect("emit rebinding Array lambda capture fixture");
    assert!(generated.contains("aura_box_ptr_retain"));
    assert!(generated.contains("aura_box_ptr_release"));
