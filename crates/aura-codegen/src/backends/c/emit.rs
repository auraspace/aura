...
fn emit_c_impl(checked: &CheckedFile, ir: Option<&CheckedIr>, opts: EmitOptions) -> String {
    let mut generic_functions = ir_generic_functions(ir, GenericOwnerKind::Function);
    for entry in &checked.mono_funs {
        if !generic_functions.contains(entry) {
            generic_functions.push(entry.clone());
        }
    }
    // Generic method bodies can call std.json.encode<T>; that intrinsic is
    // not represented in the IR's free-function instantiation list. Add the
    // concrete method argument types so its normal typed emitter is available.
    if checked.ast.functions.iter().any(|f| {
        lookup_std_intrinsic(&fun_decl_package(f, checked), &f.name.name)
            .is_some_and(|spec| spec.intrinsic == StdIntrinsic::Json)
    }) {
        for (_, _, _, method_args) in &checked.mono_methods {
            if method_args.iter().any(Ty::is_open) {
                continue;
            }
            let entry = ("encode".to_string(), method_args.clone());
            if !generic_functions.contains(&entry) {
                generic_functions.push(entry);
            }
        }
    }
    let mut generic_async_functions = ir_generic_functions(ir, GenericOwnerKind::AsyncFunction);
    for entry in &checked.mono_async_funs {
        if !generic_async_functions.contains(entry) {
            generic_async_functions.push(entry.clone());
        }
    }
    ...
    out.push_str("const char *aura_json_encode_int(int64_t value);\n");
    out.push_str("const char *aura_json_encode_bool(bool value);\n");
    out.push_str("const char *aura_json_encode_null(void);\n");
    out.push_str("const char *aura_json_encode_object(const char **keys, const char **values, int64_t count);\n");
    out.push_str("const char *aura_json_encode_array(const char **values, int64_t count);\n");
    out.push_str("const char *aura_json_encode_variant(const char *variant, const char **keys, const char **values, int64_t count);\n");
    ...
    out.push_str("const char *aura_tcp_last_error(void);\n");
...
pub(crate) fn emit_async_fun_decl(
    ...
    let emitted_std_net = lookup_std_intrinsic(
    ...
    let emitted_std_tls = lookup_std_intrinsic(
        &async_fun_decl_package(lowered, checked),
        &lowered.name.name,
    )
    ...
    let emitted_std_http = lookup_std_intrinsic(
        &async_fun_decl_package(lowered, checked),
        &lowered.name.name,
    )
    .is_some_and(|spec| spec.intrinsic == StdIntrinsic::HttpServe)
...
fn emit_fallback_unit_join_result(out: &mut String, checked: &CheckedFile) {
    let has_std_task_error_decl = checked.ast.enums.iter().any(|e| {
        lookup_enum(
            ...
            &e.name.name,
        ) == Some(StdEnumIntrinsic::IoTaskError)
    });
    let has_std_result_decl = checked.ast.enums.iter().any(|e| {
        lookup_enum(
            ...
            &e.name.name,
        ) == Some(StdEnumIntrinsic::IoResult)
    });
...
fn async_cfg_assignment_code(
    ...
    if key == "String" {
        ...
        // Evaluate the RHS before releasing the old destination. String
        // concatenation returns a fresh allocation, so transfer it directly.
        if matches!(init, Expr::Binary(_)) {
            return format!(
                "{{ const char *__cfg_string_rhs = {value}; if ({name}__owned && {name} != NULL) free((void *){name}); {name} = __cfg_string_rhs; {name}__owned = true; }}"
            );
        }
        return format!(
            "{{ const char *__cfg_string_rhs = {value}; if ({name}__owned && {name} != NULL) free((void *){name}); {name} = __cfg_string_rhs; {name}__owned = {}; }}",
...
fn emit_async_fun_std_io_fd(
    ...
    let intrinsic = lookup_std_intrinsic(&async_fun_decl_package(f, checked), &f.name.name)
        .map(|spec| spec.intrinsic);
    if !matches!(intrinsic, Some(StdIntrinsic::Io | StdIntrinsic::IoFd)) || f.params.len() != 2 {
        return false;
    }
...
fn emit_async_fun_std_tls(out: &mut String, f: &AsyncFunDecl, checked: &CheckedFile) -> bool {
    ...
    if lookup_std_intrinsic(&package, &f.name.name).map(|spec| spec.intrinsic)
        != Some(StdIntrinsic::Tls)
        || f.params.len() < 2
    {
        return false;
    }
...
fn emit_bounded_spawn_string_return_poller(
    ...
    let _ = writeln!(
        out,
        "static void {destroy_result}(void *data, size_t size) {{ (void)size; if (data != NULL) {{ const char **result = (const char **)data; free((void *)*result); free(result); }} }}\n"
    );
    let _ = writeln!(
        out,
        "static AuraTaskPollState {poll}(AuraTaskFrame *frame) {{"
    ...
    let _ = writeln!(
        out,
        "  const char *result = {value_code}; if (result == NULL) return AURA_TASK_FAILED; const char **result_box = (const char **)malloc(sizeof(*result_box)); if (result_box == NULL) return AURA_TASK_FAILED; *result_box = result; aura_task_frame_set_result(frame, (void *)result_box, sizeof(*result_box), {destroy_result}); return AURA_TASK_COMPLETE;\n}}\n\n"
    );
}
...
fn emit_bounded_spawn_await_poller(
    ...
    if let Some(key) = typed_return_key {
        if key == "String" {
            let _ = writeln!(
                out,
                "      const char *__aura_result_text = {}; if (__aura_result_text == NULL) return AURA_TASK_FAILED; const char **__aura_result = (const char **)malloc(sizeof(*__aura_result)); if (__aura_result == NULL) return AURA_TASK_FAILED; *__aura_result = __aura_result_text; aura_task_frame_set_result(frame, (void *)__aura_result, sizeof(*__aura_result), {result_destroy}); return AURA_TASK_COMPLETE;",
...
fn collect_fun_tys(checked: &CheckedFile) -> Vec<Ty> {
    ...
    // Type aliases and imported declarations are already expanded in checked
    // function signatures. Walking their AST refs would lose package identity.
    ...
    // Use resolved class signatures here. Walking AST type refs loses the
    // declaring package of imported types such as std.http.Request.
    for class in &checked.classes {
        for field in &class.fields {
            from_semantic_ty(&field.ty, &mut out);
        }
        for method in class.methods.values() {
            for param in &method.params {
                from_semantic_ty(param, &mut out);
            }
            from_semantic_ty(&method.ret, &mut out);
        }
    }
    out
}
...
fn emit_fun_typedefs(out: &mut String, checked: &CheckedFile) {
    ...
    let uses_http_server_lowering = checked.ast.async_functions.iter().any(|fun_decl| {
        lookup_std_intrinsic(
            &async_fun_decl_package(fun_decl, checked),
            &fun_decl.name.name,
        )
        .is_some_and(|spec| spec.intrinsic == StdIntrinsic::HttpServe)
    ...
    for (name, args) in &checked.mono_classes {
        let Some(class) = checked.ast.classes.iter().find(|candidate| {
            candidate.name.name == *name
                && lookup_type(&candidate.origin_package, &candidate.name.name)
                    == Some(StdTypeIntrinsic::SyncLazy)
        }) else {
            continue;
        };
        let _ = class;
