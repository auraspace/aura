...
fn json_field_name(field: &FieldDecl) -> String {
    field
        .attributes
        .iter()
...
fn json_decode_unit_enum_for_mono<'a>(
    full: &str,
    checked: &'a CheckedFile,
) -> Option<&'a EnumDecl> {
...
fn emit_json_decode_primitive_array_level(
    out: &mut String,
    array_key: &str,
    json_expr: &str,
    depth: usize,
...
fn emit_c_impl(checked: &CheckedFile, ir: Option<&CheckedIr>, opts: EmitOptions) -> String {
    ...
    if checked
        ...
        .functions
        .iter()
        .any(|f| fun_decl_package(f, checked) == "std.json" && f.name.name == "encode")
    {
...
fn json_encode_key_supported(key: &str, checked: &CheckedFile, depth: usize) -> bool {
    if depth > 32 {
        return false;
    }
