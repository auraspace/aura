//! Emit C99 for Aura C0–C1b programs and shell out to a C compiler.
//!
//! Classes lower to C structs + constructor/`this` methods.
...
pub fn emit_c(checked: &CheckedFile) -> String {
    ...
    out.push_str("/* generated by aura-codegen (C1b C backend) */\n");
    ...
    // Forward decls: constructors, methods, free functions
    for c in &checked.ast.classes {
    ...
    // Definitions: ctors, methods, free functions
    for c in &checked.ast.classes {
...
fn c_type_ref(ty: &TypeRef, checked: &CheckedFile) -> String {
    match (ty.name.name.as_str(), ty.nullable) {
        ...
        (name, true) if checked.ast.classes.iter().any(|c| c.name.name == name) => {
            // nullable class not fully supported — use pointer placeholder
...
fn emit_stmt(out: &mut String, stmt: &Stmt, indent: usize, ctx: &mut EmitCtx<'_>) {
    ...
    match stmt {
        Stmt::Var(v) => {
            ...
            ctx.define_local(&v.name.name, ty_name);
            let _ = writeln!(
                out,
                "{p}{ty} {} = {};",
                mangle_ident(&v.name.name),
                emit_expr(&v.init, ctx)
            );
        }
        Stmt::If(i) => {
...
fn infer_c_type_from_expr(e: &Expr, checked: &CheckedFile) -> String {
    // Used only when no annotation; prefer constructor / known shapes
    match e {
...
fn emit_call(c: &CallExpr, ctx: &EmitCtx<'_>) -> String {
    ...
    if let Expr::Field(fe) = c.callee.as_ref() {
        let class = resolve_class_of_expr(&fe.object, ctx).unwrap_or("Unknown");
