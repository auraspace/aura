...
fn aurac_runs_hello_example_end_to_end() {
    ...
    assert!(
        stdout.contains("Hello, Aura!"),
        "missing greeting: {stdout}"
    );
    assert!(
        stdout.contains("1231231312"),
        "missing integer output: {stdout}"
    );
