...
- Current release line: **`0.1.1-alpha.7`** after `0.1.0-alpha`; the CLI supports `new` / `init` / `check` / `build` / `run` / `test` / `race` / `fmt` / `emit-c` / `update` / `lsp` / `version` (C backend + embedded runtime).

See also the [roadmap](./roadmap.md) notes and [0.1.1-alpha.7 release notes](../releases/0.1.1-alpha.7.md).
