...
| G3  | Runtime & Integration                   | Scheduler-integrated file and TCP operations with completion/cancellation handles       | Async file/TCP Aura fixtures, peer failure, backpressure, and no double-close sanitizer evidence | In progress — bounded operation handles landed                                |
...
| G6  | Tooling + Release                       | Production registry compatibility, trust/signing policy, and cross-host acceptance      | Network publish/update smoke, signature verification, Linux arm64/Windows policy evidence        | In progress — fail-closed target/signing policy landed                        |
