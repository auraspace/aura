...
cache extraction, and offline fixtures. Direct GitHub/Git sources now resolve
immutable tags/revisions and pin source/checksum identity; workspaces remain
deferred. A proxy is explicitly later work after the read contract stabilizes.
See RFC-005 and the v0.1.1-alpha contract matrix.
...
3. **Read:** resolve `vX.Y.Z` tags and fetch the tagged source/archive directly; verify sha256 and pin the commit. Locked consumption and local round-trip acceptance are implemented.
...
- K1/K1b: direct Git origin + tag/revision cache + semver — landed; real public-host rehearsal remains an external acceptance gate
