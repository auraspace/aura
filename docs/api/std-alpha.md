...
| `std.crypto`                                          | Bounded baseline            | Secure random buffers, MD5/SHA-256, HMAC-SHA256, PBKDF2-HMAC-SHA256, constant-time comparison, and verified TLS client are runtime-backed                                                                                                                                                                                                                   |
| `std.bytes`                                           | Binary baseline             | Validated `Byte`, owned `Buffer` append/read/slice/concat, binary file reads and atomic writes, endian helpers, and literal-separator string splitting                                                                                                                                                                                                      |
| `std.net`                                             | Bounded implementation      | Nonblocking TCP handles with exact binary read/write, EOF versus partial EOF classification, absolute deadlines, and cancellation cleanup that closes pending streams                                                                                                                                                                                       |
| `std.http`                                            | Bounded implementation      | HTTP/1.1 server/client accessors, binary response bodies, and a C-backend bounded `Content-Length` fetch intrinsic for plain or verified HTTPS upstreams                                                                                                                                                                                                    |
...
| `std.tls`                                             | Bounded implementation      | OpenSSL-backed client handshake, hostname verification, certificate loading, binary/String stream adapters, and deadline-aware I/O                                                                                                                                                                                                                          |
...
| `std.json` bounded value core                         | Bounded implementation      | Validation, escaping, root classification, clone, byte length/depth, bounded options, typed outcomes, traversal, recursive `decode<T>`, and compact aggregate `encode<T>`/`stringify<T>` for primitives, nested classes/structs, arrays, string-key maps, and payload enums are implemented within documented shape limits                                  |
| `std.json` traversal/mapping                          | Bounded implementation      | `Value.get`, `at`, `asString`, `keys`, reject-duplicate policy, primitive decode, recursively nested primitive arrays, recursive nested class/struct mapping (including generic class instantiations and nullable primitive/reference/aggregate fields), unit-enum fields, and primitive/class/unit-enum arrays are implemented over owned validated slices |
| `std.stream`                                          | Bounded implementation      | Owned TCP reader/writer adapters expose String compatibility, binary Buffer exact read/write, timeout deadlines, and typed `Outcome` read/write/close boundaries                                                                                                                                                                                            |
