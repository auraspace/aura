...
| `std.json` bounded value core                         | Bounded implementation      | Validation, escaping, root classification, clone, byte length/depth, bounded options, typed outcomes, traversal, and primitive/primitive-array/recursive-class `decode<T>` are implemented within documented shape limits                                                                                        |
| `std.json` traversal/mapping                          | Bounded implementation      | `Value.get`, `at`, `asString`, `keys`, reject-duplicate policy, primitive decode, primitive arrays, recursive nested class/struct mapping (including generic class instantiations and nullable class fields), unit-enum fields, and primitive/class/unit-enum arrays are implemented over owned validated slices |
