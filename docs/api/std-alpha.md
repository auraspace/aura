...
| `std.reflect`                                         | Bounded type metadata                  | `typeOf`, `typeIdOf`, primitive `typeInfo`, reflectability, and member names are compiler-backed; member type strings remain `Unknown`                                                         |
| `std.tls`, `std.udp`, `std.websocket`                 | Locked placeholder                     | Transport/protocol shapes exist; backends and parsers are not wired                                                                                                                            |
...
| `std.json` bounded value core                         | Bounded implementation                 | Validation, escaping, root classification, clone, byte length/depth, bounded options, typed outcomes, traversal, and primitive/String `decode<T>` are implemented; richer mapping remains open |
| `std.json` traversal/mapping                          | Bounded implementation                 | `Value.get`, `at`, `asString`, `keys`, and reject-duplicate policy are implemented over owned validated slices; generic `decode<T>` remains reserved                                           |
