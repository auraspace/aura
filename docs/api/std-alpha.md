...
| `std.json` traversal/mapping                          | Bounded implementation      | `Value.get`, `at`, `asString`, `keys`, reject-duplicate policy, primitive decode, and recursive nested class mapping (including generic class instantiations and nullable class fields) with `Int`/`Bool`/`String` leaves are implemented over owned validated slices |
...
traversal/mapping (arrays, enums, and arbitrary aggregate leaves), and release evidence on deferred targets. Those gaps remain
