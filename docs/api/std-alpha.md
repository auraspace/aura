...
| `std.json` traversal/mapping                          | Bounded implementation      | `Value.get`, `at`, `asString`, `keys`, reject-duplicate policy, primitive decode, and one-level nested non-generic classes with `Int`/`Bool`/`String` fields are implemented over owned validated slices |
...
concurrent tracing collector, cryptographic/TLS backends, deeper generic JSON
