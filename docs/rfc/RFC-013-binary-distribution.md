...
The current required artifact suffixes are `linux-amd64`, `linux-arm64`,
`darwin-arm64`, and `darwin-amd64`; `windows-amd64` and `windows-arm64` remain
tier2 policy rows and have no release assets yet.
...
| 3   | Hosting URL / CDN  |                | Project | **Resolved** — toolchain via GitHub Releases + docs site; packages use direct Git origins and immutable semver tags per RFC-005 |
