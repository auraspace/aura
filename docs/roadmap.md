...
| Track                       | Status                                                                                                                                                                                                                  |
| --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| RFC static site (`site/`)   | Implemented; Cloudflare Pages → **https://auraspace.github.io/aura**                                                                                                                                                    |
...
- Deploy to GitHub Pages (`VITE_BASE=/aura/`, repository path `auraspace.github.io/aura/`)
...
| **C12l**   | Lambda capture Array (owned snapshot / retained mutable cell)            | **Done**                                                                                                                                                       |
...
| **C13i**   | Direct origin client MVP (fixture + cache)                               | **Done**                                                                                                                                                       |
| **C13j**   | Semver caret resolve → lock pins                                         | **Done**                                                                                                                                                       |
| **C13k**   | Fetch tagged source/archive + sha256 + extract cache                     | **Done**                                                                                                                                                       |
| **C13l**   | `aura build`/`check` with locked origin deps                             | **Done**                                                                                                                                                       |
...
| **C20d**   | Mutable `var` Array capture (shared owned cell + captured Array payload) | Done — shared-cell ownership, mutation visibility, owner movement, and escaping-closure cleanup are native-tested                                              |
...
| **C22a–d** | Freeze async/task contracts, syntax, types, and borrow barriers          | **Done** — `d8e4640`, `9f01b8e`, `3a5f9d1`, `79c4a32`                                                                                                          |
| **C22e–g** | Add AST/lexer and parse async/task/channel operations                    | **Done** — `4bd0127`, `fab198b`, `1b48d64`                                                                                                                     |
| **C22h–i** | Type-check async operations and enforce borrow barriers                  | **Done** — `770f935`, `cd645d5`                                                                                                                                |
| **C22j–k** | Add task-frame ABI and deterministic executor                            | **Done** — `84d0e81`, `695d760`                                                                                                                                |
| **C22l–m** | Lower async bodies and task operations                                   | **Done** — general CFG suspension/resumption, spawn captures, typed frame ownership, and cancellation/task outcome propagation are covered by executable tests |
| **C22n–o** | Add bounded channels and typed channel lowering                          | **Done for MVP slice** — Int/String/class payloads; `1064aeb`, `4d4929f`                                                                                       |
| **C22p–s** | Add corpus, diagnostics, churn, and ownership/GC audit                   | **Done** — async branch/loop/capture/outcome fixtures, ownership scans, and sanitizer gates are recorded                                                       |
...
* C22 (closed): [`docs/plans/2026-07-22-next-20-c22-async-tasks.md`](plans/2026-07-22-next-20-c22-async-tasks.md)
...
1. ~~Language surface through C22~~ (funs/lambdas, mutable captures, scoped ref T, borrow-safe field returns, async state machines, captures, outcomes, and channels)
2. Runtime: ~~GC + process I/O + String Array free + Fun env RC~~ → C22 deterministic executor/channels, task storage scans, typed causes, FFI pin retention, worker/reactor paths, and frame ownership landed; concurrent tracing GC remains outside this release contract
