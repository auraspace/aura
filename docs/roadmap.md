...
| Compiler | **C0–C3f slice** — + `import` / `pub` / path deps |
...
1. ~~Generics + interfaces + bounds~~ · ~~struct~~ · ~~enum/match/Result~~ · ~~exceptions~~ · ~~`aura test`~~ · ~~multi-file / `aura.toml`~~ · ~~`import` / path deps (C3f)~~ → next: class throws / registry / LLVM
