...
| Compiler                    | **C0–C20** closed; **C21a** selects scoped borrow/ref as the next track; live ownership deferred             |
...
1. ~~Language surface through C20~~ (funs/lambdas, mutable class/Array/Fun capture MVP) → **C21 borrow/ref** for safe field returns/views; later: Array-of-iface, async/tasks
...
4. Stdlib: ~~io + collections + C13 toString/case/eprint/tryWrite + C14 generic HashMap + C15 generic HashSet + C18 hash-collection HOFs + C19 accessors/entry snapshots/entry for-in + C20 snapshot iterators~~ → C21 borrow/ref enables later live iterator/entry-view APIs; entry mutation remains deferred
