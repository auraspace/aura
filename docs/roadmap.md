...
| **C22l–m** | Lower async bodies and task operations                             | **Partial** — no-await/empty-spawn slice; await state machine, captures, and full failure propagation deferred (`5354487`, `185a3c7`) |
| **C22n–o** | Add bounded channels and typed channel lowering                    | **Done for MVP slice** — Int/String/class payloads; `1064aeb`, `4d4929f`                                                              |
| **C22p–s** | Add corpus, diagnostics, churn, and ownership/GC audit             | **Done with residual debt** — `b86fc7c`, `4a6fa78`, `c919aef`, `edd1350`                                                              |
...
- C22 (partial implementation closed): [`docs/plans/2026-07-22-next-20-c22-async-tasks.md`](plans/2026-07-22-next-20-c22-async-tasks.md)
...
1. ~~Language surface through C21~~ (funs/lambdas, mutable captures, scoped `ref T`, borrow-safe field returns) → C22 surface/borrow rules landed; await lowering and non-empty spawn captures remain deferred; Array-of-iface later
2. Runtime: ~~GC + process I/O + String Array free + Fun env RC~~ → C22 deterministic executor/channels landed; OS-thread scheduler, blocking-I/O reactor, concurrent GC, and task-frame GC roots remain later work; fix `Io.args` strdup vs free
...
S2 production toolchain implementation: [`docs/plans/2026-07-21-s2-production-toolchain.md`](plans/2026-07-21-s2-production-toolchain.md). Release publication remains pending and is not requested by C21.
